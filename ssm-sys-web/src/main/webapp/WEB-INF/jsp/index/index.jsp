...
<script type="text/javascript">
    ...
    window.onload = function () {
        ...
        $.ajax({
            type: "post",
            url: "<%=request.getContextPath()%>/permiss/ownlist.do",
            success: function (result) {

                var ztreedata = fn(result, "");
                var treeObj = $("#treeDemo");
                $.fn.zTree.init(treeObj, setting, ztreedata);
    ...
    function fn(data, pid) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].isValid == "A" && data[i].permissType == "MENU") {
                var obj;

                var type = data[i].permissVal.indexOf("click") > 0;
